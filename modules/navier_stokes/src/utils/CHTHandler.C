...
CHTHandler::validParams()
{
  ...
  params.addParam<MooseFunctorName>(
      "thermal_resistance",
      "0",
      "The custom area-normalized thermal resistance at the interface, in m^2*K/W.");
...
CHTHandler::CHTHandler(const InputParameters & params)
  : MooseObject(params),
    ...
    _thermal_resistance(getFunctor<Real>("thermal_resistance"))
{
  ...
  for (const auto bd_index : index_range(_cht_boundary_ids))
  {
    ...
    for (const auto & fi : bd_fi_container)
    {
      ...
      const auto thermal_resistance =
          _thermal_resistance(Moose::FaceArg{fi,
